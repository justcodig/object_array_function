...
2. 화살표 함수
- 화살표 함수는 기존의 방식보다 상대적으로 함수를 좀더 간결하게 표현하는 새로운 방식이다.
...
- 생성자로도 사용 못함
 ...
 ### 주의할 점
 - 화살표 함수는 많은 장점을 제공하지만,모든 상황에서 기존 함수를 완변하게 대체할 수있는 것은 아니다.
 - 특정상황에서는 화살표 함수의 특성이 문제를 일으킬수도 있으므로 사용에 주의가 필요하다.
...
let t = (a,b,c) => "요건 return이됨 화살표의 축약형";
console.log(t());

let ar = name => "이것은 소괄호 ${name}입니다.";
consol.log(ar("생략"));
출력 이것은 소괄호 생략입니다.
...
- 내장(built-in)객체는 자바스크립트 시스템에서 정의된 객체로 그냥 불러서 사용할 수 있다. 데이터 관련 객체와 브라우저 관련 객체,HTML 관련 객체(DOM)이 있다.
- 모든 자바스크립트 환경이 이용가능한 핵심적인 객체로 자바스크립트에서 자체적으로 지원하는 것이다.

### 데이터 관련 객체
- String 객체(문자열 값은 그 자체가 하나의 객체가 된다.문자열을 조작할수 있는 기능을 제공한다.)

- Number 객체 ( 자바스크립트에서 숫자 데이터를 처리하기 위한 내장 객체입니다.)

- Date 객체 (시스템의 현재날짜,시간을 조회하거나 설정하기 위한 기능을 제공한다.)

- Boolean 객체 (boolean value를 객체화 시킨것)

- Array 객체 (같은 종류의 변수를 하나로 묶기 위한 배열에 관련된 기능을 제공한다.)

- math (수학 계산을 위해 기본적으로 제공되는 객체)

### 브라우저 관련 객체(BOM)
- window 객체 (브라우저 창에 대한 모든 상항을 제어하는 최상위 객체이다.)

- Navigator 객체 ( 브라우저의 종류를 판별한다.)

- Screen 객체 (브라우저 화면에 대한 정보를 알려준다.)

- History 객체 (웹 브라우저에 기록되어 있는 히스토리 정보를 제어한다.)

- Location 객체 (url정보를 제어하는 객체이다. 페이지이동, 새로고침 등의 기능을 제공한다.)

### BOM(Browser Object Model)
- BOM(브라우저 객체 모델)은 브라우저와 컴퓨터 스크린에 접근할 수 있는 객체의 모음을 말한다. BOM은 window를 제어하며, window 객체를 통해 접근이 가능하다
